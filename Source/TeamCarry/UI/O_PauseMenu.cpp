...
void UO_PauseMenu::HandleLeaveRoomClicked()
{
	if (UMockUIController* MockController = GetGameInstance()->GetSubsystem<UMockUIController>())
	{
		// 파괴적 액션(세션 파기 또는 본인 이탈)이므로 직접 전환하지 않고 확인 모달을 푸시한다.
		// 호스트가 누르면 세션(NAME_GameSession) 자체가 파기되어 다른 플레이어 전원이 함께
		// 끊기므로, 클라이언트와 다른 경고 문구를 보여준다(LeaveToTitle()은 호출자 권한에 따라
		// 이미 다르게 동작함 — OnConfirmLeaveRoom 참고).
		...
		if (UO_Confirm* ConfirmUI = Cast<UO_Confirm>(OverlayWidget))
		{
			...
			ConfirmUI->SetupConfirm(
				...
				bIsHost
					? FText::FromString(TEXT("정말로 방을 나가시겠습니까? 진행 중인 스테이지가 종료되며, 모든 플레이어가 게임에서 나가게 됩니다."))
...
void UO_PauseMenu::OnConfirmLeaveRoom()
{
	...
	// 이동한다 — 호스트가 호출할 때만 실제로 세션 전체가 파기되고(HandleLeaveRoomClicked()가
	// 그 경우 경고 문구를 다르게 보여주는 이유), 클라이언트가 호출할 때는 자신만 이탈할 뿐
	// 다른 플레이어의 세션/스테이지 진행에는 영향이 없다. 이 비대칭 동작 덕분에 Btn_LeaveRoom을
	// 호스트/클라이언트 구분 없이 그대로 노출할 수 있다(2026-07-15 개정).
	if (UTCSessionFlow* Flow = GetGameInstance()->GetSubsystem<UTCSessionFlow>())
	{
...
void UO_PauseMenu::RefreshContextVisibility()
{
	...
	{
		// Btn_Save/Btn_ToLobby/Btn_Reset: 멀티플레이 동기화를 위해 호스트(방장) 전용. 노출 분기는
		// 편의일 뿐이므로 실제 트래블/저장/리셋 실행은 서버 측에서 재검증한다(명세 6장-8).
		...
		// Btn_LeaveRoom: 호스트/클라이언트 모두 노출(2026-07-15 개정). LeaveToTitle()이 호출자
		// 권한에 따라 이미 다르게 동작하므로(호스트=세션 파기, 클라이언트=본인만 이탈), 게이팅 없이
		// 그대로 노출하고 HandleLeaveRoomClicked()의 확인 문구로만 영향 차이를 알린다.
		if (Btn_LeaveRoom) Btn_LeaveRoom->SetVisibility(ESlateVisibility::Visible);
	}
}
