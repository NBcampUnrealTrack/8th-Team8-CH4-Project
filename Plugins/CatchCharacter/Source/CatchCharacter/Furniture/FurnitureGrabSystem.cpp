...
// 운반 이벤트 로그 — 화면 디버그(F9)와 달리 항상 기록되어, PIE 종료 후 Output Log만으로
// 그랩/해제·견인 전이·후퇴·밸브·자동해제 흐름을 재구성할 수 있다 ("LogCarry"로 검색).
DEFINE_LOG_CATEGORY_STATIC(LogCarry, Log, All);

namespace
{
	// F9(TC.GrabDebug)와 같은 스위치 공유 — 운반 시각화(견인/막힘/후퇴/밸브/자동해제) 게이트.
	// CVar 자체는 TeamCarry 모듈(GrabComponent)이 등록하므로 여기서는 조회만 한다.
	// (모듈 로드 순서상 첫 조회가 등록보다 빠를 수 있어 null이면 캐시하지 않고 재시도)
	...
	// 조준 피치를 [-90,90]으로 접기 — 원격 피치(RemoteViewPitch)는 ±90 밖 랩 값이 올 수
	// 있어(예: 144°) 높이 목표가 수백 uu씩 튀는 수직 텔레포트가 됐음
...
void UFurnitureGrabSystem::Grab(ACharacter* Grabber, FVector height, UPrimitiveComponent* GrabberComponent)
{
	...
	if (GrabbedPlayers.Num() == 0 && FurnitureMesh)
	{
		...
		// [스택 웨이크] 겹치거나 얹힌 시작-정적 가구에 물리를 켠다 — 정적인 채로 두면
		// 이 가구가 그 밑에서 빠져나오지 못하고 매 틱 시작-관통 → 밸브 루프가 된다
		// (침대 프레임 위 정적 매트리스, 밸브 637회/0.5s 로그).
		{
		...
		// [끌기 가구는 초기 리프트 생략] 2인 가구를 혼자 잡으면 어차피 틱에서 끌림 자세
		// (바닥 접지)로 내려가는데, 여기서 허리 높이로 들었다가 내려가면 '들썩'하는 팝이 됨.
	...
	// [피동 플래그 초기화] 인원 구성이 바뀌면 이전 대형 기준의 피동/도달 추적은 무효.
	// 특히 솔로 끌기 중 침범 판정으로 남은 drag 플래그가 합류 후로 이월되면, 좁아진 도달
	// 반경(히스테리시스) 안에 갇힌 채 견인 주입이 이동 입력을 매 틱 덮어써 '먼저 잡은
	// 사람이 못 걷는' 교착이 됨 — 솔로에선 WTotal=0 리셋으로 풀리지만, 합류자가 가중치를
	// 공급하는 2인부터는 그 리셋이 영영 안 걸린다.
	...
	{
		FGrabAnchor Anchor;
		// [자연 좌표 기준] 앵커는 높이 오프셋(인원미달 끌림·카메라 높이 조절)을 뺀 자연 좌표로
		// 기록해야 한다. 실제 위치로 기록하면 HandleMovement가 목표 위치에 오프셋을 한 번 더
		// 더해 — 특히 끌림 자세 중 두 번째 운반자가 합류할 때 — 하강분이 이중 적용되어
		// 가구가 바닥을 파고들고, 바닥 스윕 막힘·견인이 매 틱 충돌하며 덜덜거린다.
...
void UFurnitureGrabSystem::Release(ACharacter* Grabber)
{
	...
	// [남은 운반자 재앵커] 한 명이 놓는 순간, 가구가 남은 사람의 '옛 형성 위치'로
	// 미끄러져 이동하는 것 방지 — 현재 가구 상태를 남은 운반자들의 새 기준으로 굳힌다.
	...
	if (GrabbedPlayers.Num() == 0 && FurnitureMesh)
	{
		...
		// 우선 무조건 +3: 끌림 자세의 접촉(+1)·얕은 관통은 겹침 검사에 안 걸릴 수 있어
		// 소량 들어올리고 물리에 맡기면 중력이 자연 안착시킨다 (바닥 박힘 방지).
		...
		// CCD: 기울어진 자세에서 놓으면 접지 모서리를 축으로 빠르게 회전 낙하하는데,
		// 이산 충돌만으로는 가는 다리가 얇은 바닥을 통과(터널링)해 박힌 채 안착함 —
		// 박힌 가구는 스윕이 전부 거부되어 다시 잡아도 들리지 않는 잠금으로 이어진다.
...
void UFurnitureGrabSystem::HandleMovement(float DeltaTime)
{
	...
	// [운반 속도 서버 강제] 달리기 토글·RPC 레이스 등 어떤 경로로 덮어써져도(디버그에서
	// 운반 중 MaxWalkSpeed 500 목격) 서버가 매 틱 운반 감속을 재적용해 권위를 유지한다.
	{
	...
	// ---- 0.7. [운반 거리 테더] ----
	// 가구 바운즈의 최근접 지점이 운반자에게서 MaxReach 이상 벗어나 있으면(막힘·보정·피벗
	// 편차의 누적) 앵커 오프셋을 초과분만큼 운반자 쪽으로 틱당 소량씩 재기록 — '들고 있다'는
	// 시각적 거리를 항상 유지하고, 벌어진 상태가 스스로 회복되게 한다. (1인 운반 한정)
	if (N == 1 && FurnitureMesh)
	{
		...
		if (GapLen > MaxReach)
		{
			...
			if (FGrabAnchor* Anc = Anchors.Find(Players[0]))
			{
				...
				// 주의: 여기서 Multicast_SetPlayerAnchor(Reliable)를 쏘지 않는다 — 벌어진
				// 상태가 지속되면 틱마다 발사되어 Reliable 큐 폭주(연결 끊김) 위험.
				// 클라이언트는 이 XY 오프셋을 배치에 사용하지 않으므로(Yaw 기준값만 사용)
				// 생략해도 무해하고, 다음 정식 앵커 재기록 때 자연히 동기화된다.
	...
	for (int32 i = 0; i < N; ++i)
	{
		...
		// [들것 회전 — 걷기 기반] 2인 이상은 카메라가 아니라 '두 운반자를 잇는 선'의 회전만
		// 가구 회전으로 반영한다(0.5에서 능동 이동 기여만 누적 — 카메라를 돌려도 가구는
		// 안 돌고, 한 사람이 상대를 축 삼아 걸어 돌면 가구가 두 사람 사이에서 회전).
		// Step 5의 '회전 우선 견인'과 세트: 가구가 다 돌아간 뒤에야 상대가 끌려온다 —
		// 문 통과 시 상대를 고정축으로 가구 방향을 맞추는 조작이 가능해진다.
		...
		// [들것 조향] 들것 모드에서 이동 입력 중인 운반자는 캡을 풀어 가구 위치를 사실상
		// 전담시킨다 — 캡 잔여 비율만큼 서 있는 상대의 낡은 제안이 가구를 되끌면, 축(상대)
		// 기준 회전이 어긋나고 입력자 대형 오차가 누적돼 견인 오발동('같이 걷기')이 됐다.
	...
	// [들것 회전 속도] 궤도 회전(반경 ~150uu에서 250uu/s로 걸으면 선 각속도 ≈ 95°/s)이
	// 기본 상한(90°/s)을 넘으면 가구 Yaw가 선을 계속 뒤쫓기만 하며 형성이 뒤틀려, 축(피동)
	// 쪽 끝이 제자리에 못 있고 휩쓸린다('축 회전이 잘 안 됨' 리포트) — 들것 모드는 상한 2배.
	...
	if (FurnitureHeightPerPitch != 0.0f)
	{
		...
		for (int32 i = 0; i < N; ++i)
		{
			// 절대 피치 기준(정면 0°=기본 높이), ×2 감도. ±45° 클램프: 극단 피치가
			// 목표를 수백 uu로 키워 클램프 경계를 때리는 것 방지 (상한은 8°대에 이미 도달)
	...
	// 45°+ 기울어 놓인 가구는 자세 교정·끌림 접지 대상에서 제외 — 일반 들기(피치 높이)로
	// 운반해 카메라로 들어 올릴 수 있게 하고, 놓으면 물리가 재안착시킨다.
	...
	{
		// '플레이어 반대쪽' 방향은 피벗이 아니라 메시 바운즈 중심 기준 — 피벗이 메시에서
		// 멀리 떨어진 가구(이 킷 다수)는 피벗 기준이면 잡는 방향과 무관하게 늘 같은
		// 쪽이 들리는 오동작이 남 (기울기 축·먼 쪽 판정이 전부 이 방향을 씀).
		...
		{
			// 카메라 피치 → '든 쪽' 높낮이: 가구 전체를 띄우는 게 아니라 기울기 각도를
			// 조절한다. 정면=기본 14°, 위 보면 내 쪽이 더 들리고(최대 22° — +8°면 도달),
			// 내리면 거의 평평(2°)까지. 먼 쪽 끝은 항상 바닥에 끌린다.
			// 도당 1.0: 0.45일 땐 카메라를 한참 들어야 반응이 보여 '안 들리는' 체감이었음.
			// (상한 22°: 대형 가구가 통째로 서 버리며 시야를 가리고 형성이 밀리는 것 방지)
			const float AimPitchDeg = FoldAimPitch(Players[0]->GetBaseAimRotation().Pitch);
			// 기울기 상한을 각도(22° 고정) 대신 '든 쪽 끝 들림 높이(~70uu)'로 동적 캡:
			// 작은 가구(스팬 150)는 22°=28uu밖에 안 들려 피치 반응이 안 보였음 → 최대 35°,
			// 대형 가구는 낮은 각도로 캡되어 통째로 서 버리는 것은 계속 방지.
			...
			// 상한 28°: 35°였을 때 짧은 스팬(옆 잡기)에서 실측-보정 오버슈트와 겹쳐 45°
			// 넘어짐 판정(정립=0)으로 굴러 모드가 널뛰었음 — 17° 안전 마진 확보.
			...
			// [지지면 기준 하강] 가구 바로 아래의 실제 지지면을 찾는다.
			// 접지 높이(FloorZ)는 2점(중심+끌리는 쪽 끝) 최대값 — 선반에 걸친 상태의 널뜀 방지.
			// 모드 판정(FloorZGate)은 중심점만 — 끝점까지 최대로 쓰면 선반 옆에서 끝점이
			// 선반 위에 남아 '뽑는 중(일반 운반)'이 유지돼 끌림 자세로 영영 전환 안 됨(TV 리포트).
			...
			if (FurnitureMesh)
			{
				...
				// 풋프린트 판 하향 스윕(정적 전용): 점 트레이스가 놓치는 '일부만 걸친 지지물'
				// (러그 가장자리 등)을 잡는다 — 접지 목표가 지지물 내부로 들어가면 매 틱
				// 시작-관통 → 밸브 루프(침대 637회/0.5s 로그). XY 10% 축소 + 상향 노멀만
				// 인정해 벽 측면 히트 오염을 배제.
				{
			...
			// [열린 공간 전용] 끌림 자세(강제 하강+기울기)는 지지면이 운반자 발밑과 비슷한
			// 높이일 때만 발동. 장식장 위·벽걸이처럼 높은 곳에서 뽑는 동안 하강을 걸면
			// 스텝업과 반대 방향으로 싸우며 점프(텔레포트 체감)하고, 갇힌 채 리프트 스윕이
			// 주변을 타격해 즉시 파손까지 유발 → 뽑는 동안은 일반 운반(수평·잡은 높이),
			// 낮은 곳으로 내려온 뒤부터 끌림이 시작된다. (판정은 중심점 지지면 기준)
			if (FloorZGate <= CarrierFootZ + 40.0f)
			{
				...
				// +3: 관통 없이 '닿아 보이는' 높이. 기울기 회전은 스윕이 안 되어 틱당 최대
				// ~1uu씩 모서리가 내려오는데, 유격이 그보다 작으면(+1이었을 때) 매 틱
				// 얕은 관통 → 관통 밸브 발동이 무한 반복된다(밸브 스팸 리포트). 음수 겹침을
				// 주면 그 자세로 놓았을 때 박힌 채 시작하는 것도 동일 이유.
				// 높이는 항상 'AABB 하단 ≈ 바닥' — 피치는 위(기울기 각도)에서만 반영되어
				// 든 쪽 높낮이가 변하고, 먼 쪽 끝은 계속 바닥에 끌린다 (공중부양 없음).
				// 중심 절대 상한(발밑+100): 기울어진 대형 가구의 AABB가 커져도 통째로
				// 솟아오르지 않게 — 상한에 걸리면 아랫끝이 바닥을 누르는 채 유지(스윕이 받음).
	...
	// [요건 충족 시 최소 운반 높이] 끌림 자세(바닥)나 낮은 위치에서 잡으면 앵커가 그 높이로
	// 굳어, 인원을 채워도 가구가 바닥에 깔린 채 운반되던 문제 — 인원 충족 시 가구 하단이
	// 최소 '플레이어 평균 발밑 +20' 위로 오도록 높이 오프셋의 하한을 끌어올린다 (보간이
	// 자연스럽게 들어올림). 두 번째 운반자가 합류하는 순간 함께 들어 올리는 연출이 된다.
	// '같이 내려놓기': 전원이 명확히 아래(-25° 이하)를 보고 + 가구가 실제로 들려 있을 때만.
	// 그랩 델타 방식은 내려다보며 잡으면 델타 0이라 영영 못 내렸음(로그: 피치 -54, 목표 고정).
	// '들려 있음(발밑+30uu)' 조건이 픽업 순간(바닥 가구, 자연히 내려다봄)의 오발동을 막는다.
	...
	// [하강 등속 제한] 끌림 자세에서 지지면이 끊기는 순간(선반 모서리를 벗어날 때) 목표가
	// 바닥으로 급락하는데, 비례 보간은 큰 낙차를 한숨에 따라가 '순간이동 낙하 + 충돌 파손'이
	// 됨 → 큰 하강만 초당 일정 속도로 내려가게 제한 (상승·소폭 조정은 기존 보간 유지).
	if (bUnderManned)
	{
		// 끌림 자세는 상승·하강 모두 등속 — 지지면 트레이스가 소파↔바닥처럼 오가는
		// 가장자리에서 비례 보간(빠른 상승)이 섞이면 위로 튕기며 통통 바운스가 됨.
	...
	// 경사에서 두 사람 높이가 다르면 그 평균에 맞춰 허용 범위(고저)가 함께 오르내림.
	{
		...
		// 상한은 '메시 하단 ≤ 플레이어+Max' — 중심 기준이면 TV처럼 세로로 길거나 높이 놓인
		// 가구는 서 있기만 해도 상한에 걸려 들기가 전혀 안 됨 (로그: 목표 51~133, 적용 13 고정)
	...
	// 공동운반은 정립 게이트 없이 항상 수평 복원 — 운반 중 사고로 45°+ 넘어간 자세가
	// 고착되면 낮은 모서리가 바닥을 긁어 회전 롤백→평행이동만 남는 문제(로그 정립=0 고정).
		...
		&& FurnitureMesh && FurnitureMesh->GetStaticMesh())
	{
		...
		if (PairDist > 1.0f)
		{
			...
			// B쪽 손이 높으면 B쪽 끝이 올라감. 과도한 기울기는 ±20°로 제한.
			// 손높이차는 최소 '가구 스팬(HorizDist)' 위에 펼쳐 경사로 환산 — 두 사람이 옆/같은
			// 쪽에 붙어 잡으면 분모(간격)가 작아 사소한 피치 차이도 최대 경사로 발산해, 높이
			// 조절이 회전(기울어짐)으로 둔갑하던 문제 방지. 양끝을 나눠 들면 간격≈스팬이라 동일.
			// 간격이 최소치 미만이면 경사 0(수평 복원)만 — 그래도 이 블록을 계속 돌리는 이유는
			// 끌기 단계에서 남은 잔여 기울기·측면 롤을 합류 직후에도 풀어 줘야 하기 때문
			// (예전엔 간격<최소치면 블록 전체가 꺼져 드러누운 자세가 영영 안 펴졌음).
			...
			// [측면 수평 보정] 페어 라인의 직교 성분(측면 롤)은 보정 대상이 아니라서 둘이
			// 돌며 나르는 동안 잔여 기울기가 누적됨 — 끌림 자세와 동일한 체계로 0 복원.
	...
	// 절대 자세(수평+14°)로 재구성하지 않고 '측정-보정형'으로 동작: 현재 자세에서 플레이어
	// 방향(D)의 실제 경사를 재고 목표 경사(-14°)로 초당 일정 각도만 회전 — 눕혀진 가구도
	// 잡은 쪽이 들리고, 절대 자세 스냅이 없어 원복 팝·텔레포트가 발생하지 않는다.
	else if (bUnderManned && bUprightEnough && UnderMannedTilt > 0.0f && FurnitureMesh && FurnitureMesh->GetStaticMesh())
	{
		...
		// [측면 수평 보정] 기울기 축(플레이어→가구 방향)은 이동에 따라 도는데 그 직교 성분
		// (측면 롤)은 아무도 보정하지 않아 잔여 기울기가 누적됨 — 가구 주위를 돌며 끌면
		// 목표각(≤22°)을 훌쩍 넘겨 드러눕고, 그 자세로 놓으면 모서리부터 낙하해 바닥을
		// 뚫는다. 측면 방향 경사를 실측해 0으로 복원한다 (같은 측정-보정 체계).
		...
		FQuat NewQ = FQuat(TiltAxis, FMath::DegreesToRadians(DeltaDeg))
		           * FQuat(SideAxis, FMath::DegreesToRadians(SideDelta))
		           * YawDeltaQ * Owner->GetActorQuat();
		// 안전핀: 결과가 35°를 넘게 기울면 이번 틱 기울기 성분 폐기 — 축 이동·오버슈트
		// 누적으로 45° 넘어짐 판정을 넘겨 '통째 들기' 모드로 튀던 문제(로그 정립 1→0) 차단
		if (NewQ.GetUpVector().Z < 0.82f)
		{
			NewQ = YawDeltaQ * Owner->GetActorQuat();
		}
	...
	// [관통 동결 해제 밸브] 시작 시점에 이미 정적 지오메트리와 겹쳐 있으면(끌림 기울기의
	// 얕은 파고듦, 물리 안착 잔여 관통 등) 모든 스윕이 t=0에 거부되어 가구가 영구 동결됨 —
	// 들기·끌기·스텝업 전부 무력화되고, 두 번째 운반자가 합류해도 반응이 없어진다.
	...
	if (MoveHit.bStartPenetrating && FurnitureMesh)
	{
		...
		// 정적+동적 모두 검사 — 바리케이드·다른 가구 같은 동적 오브젝트에 박힌 경우
		// 정적 전용이면 '겹침 없음'으로 즉시 탈출 성공 처리되고 실제로는 그대로 박혀
		// 매 틱 밸브가 헛돌았음(로그 스팸 리포트). 잡은 플레이어는 아래 파라미터로 무시됨.
		...
		{
			// 위쪽이 막힌 형태(선반 밑 등)는 상향 탈출이 오히려 파고듦 — 원위치 유지
	...
	// [가구 스텝업] XY 이동이 낮은 장애물(장식장·문턱·소품)에 막히면 캐릭터 스텝업처럼
	// '들어 올려 → 재시도 → 안착'으로 타고 넘는다. 벽처럼 높은 장애물은 리프트 상태에서도
	// 막히므로 순 이동 0으로 끝나 기존 차단 동작 유지. (벽걸이 TV가 장식장 틈에 걸려
	// 뽑히지 않던 문제의 해법 — 당기면 장식장 위로 타고 올라오며 빠진다.)
	{
		...
		{
			// 리프트를 낮은 것부터 점진 시도: 성공해도 필요한 만큼만 올라가 팝핑을 줄이고,
			// 실효 이동이 없으면(벽 등 못 넘는 장애물) 통째로 원위치 — 매 틱 위아래로
			// 들썩이며 드드득거리는 것 방지.
			// [자기 타격 방지] 리프트 스윕이 선반 밑면 등을 때리며 발생하는 히트가 충돌
			// 데미지로 들어가 갇힌 가구가 몇 틱 만에 파손되던 문제 — 시도 동안 잠깐 무적.
			if (UFurnitureDamage* DamageComp = Owner->FindComponentByClass<UFurnitureDamage>())
			{
				DamageComp->SetInvincible(0.25f);
			}
			const FVector PreStepPos = Owner->GetActorLocation();
			// 틱당 이동량 제한: 밀린 거리 전부를 한 틱에 옮기면 성공 순간 수십 uu가
			// 점프(텔레포트 체감)함 → 조금씩 여러 틱에 걸쳐 긁으며 미끄러지게.
	...
	// [회전 관통 방지] 회전에는 스윕이 적용되지 않아 큰 판형 가구(TV 등)가 돌 때 벽을
	// 파고듦 — 수동 회전(AddFurnitureOffset)에만 있던 겹침 검사를 운반 틱 회전에도 적용.
	// 이번 틱 회전으로 정적 지오메트리와 겹치면 회전만 직전 값으로 되돌린다(이동은 유지).
	// 검사 위치 +3uu: 지지면에 얹힌 정상 접촉을 겹침으로 오탐하지 않기 위함.
	if (FurnitureMesh && !PreMoveRot.Equals(Owner->GetActorRotation(), 0.05f))
	{
		...
		if (bPenetrates)
		{
			// 전량 롤백하면 장애물 옆에서 회전이 통째로 얼어붙고, 카메라만 돌아가
			// 가구가 반대로 움직이는 듯한 착시('반전' 리포트)가 됨 → 절반·1/4로
			// 부분 회전을 시도해 닿기 직전까지는 따라오게 한다.
	...
	// [피벗-중심 기울기 보정] '확정된' 회전 변화 기준으로 메시 중심 변위를 상쇄.
	// 회전 적용 전에 보정하면 관통 롤백이 회전만 취소하고 보정 이동은 남아, 매 틱
	// 순수 이동이 누적되며 가구가 플레이어에게서 멀어지는 드리프트가 발생했음(순서 버그).
	// 요 성분은 앵커 시스템이 피벗 기준이라 제외하고, 기울기(피치/롤) 변화분만 상쇄한다.
	// [1인 전용] 이 보정은 피벗을 이동시키므로, 피벗 기준으로 형성(앵커·견인 목표)을 잡는
	// 2인 운반에서는 기울기가 변할 때마다 견인 목표가 흘러 두 운반자가 끌려 겹치는 충돌이
	// 발생 — 공동운반에서는 끄고 형성 안정을 우선한다.
	...
	// InitAimYaw 유지 → ProposalYaw = ActualYaw + 원래카메라각도 → 막힘 해제 후 즉시 정상 회전.
	{
		...
		if (bRotationBlocked)
		{
			if (bPairLineValid)
			{
				// [들것 회전] 의도 리셋만으로 충분 — 다음 틱 전원의 ProposalYaw가 현재 Yaw와
				// 일치해 YC가 상수가 되므로 호(弧) 미끄러짐이 애초에 생기지 않는다.
				// 위치 재앵커는 하면 안 됨: 들것 모드에선 동료가 걷는 동안 선 회전 의도가
				// 매 틱 쌓이고 가구 회전은 속도 제한·벽 롤백으로 뒤처져 이 블록이 만성
				// 발동하는데, 매 틱 오프셋을 현재 위치로 다시 구우면 피동 운반자의 견인
				// 거리(Delta)가 누적되지 못해 데드존을 못 넘고 — 동료가 가구를 들고 걸어가도
				// 제자리에 남는 '안 따라옴'이 된다.
	...
	// '위로 막힘'(목표 > 실제)만 재기록한다. 예전처럼 양방향으로 재기록하면, 스텝업·충돌로
	// 잠깐 올라간 높이가 앵커에 구워져 새 기준이 되어 카메라 기준 높이로 다시 안 내려오는
	// 문제가 생김 — 실제가 목표보다 높은 경우는 그대로 두면 장애물을 벗어나는 즉시
	// 스윕이 목표(카메라 기준)로 자연 하강시킨다.
	...
	if (bBlockedCarrierStopsFurniture)
	{
		...
		if (WorstBlock.SizeSquared() > FMath::Square(BlockStopThreshold))
		{
			if (IsCarryDebugEnabled() && GEngine)
			{
				// 가구 후퇴 발동 — 이게 매 틱 반복되면 운반 전체가 러버밴딩으로 얼어붙는다
	...
	for (ACharacter* P : Players)
	{
		...
		// [들것 한계 견인] 진입(한계)은 넓게 — 상대가 축으로 돌 때 대형 목표가 주변에서
		// 흔들려도 견인이 안 걸려 제자리 축이 유지된다. 일단 한계를 넘어 팽팽해지면 대형
		// 근처(Exit)까지 '연속 추종'한다 — 이전의 슬랙 유지 방식(60에서 도달 정지 → 90까지
		// 벌어지면 재출발)은 스톱-앤-고로 덜컹이며 딸려오는 기차 체감이 됐음(로그: 시작 91→
		// 도달→시작 91 반복). 연속 추종이면 리더가 걷는 동안 램프 지연(~30uu)만 뒤에 붙는다.
		const float PairHoldRadius = 70.0f;   // 첫 진입(한계) — 축 회전 중 목표 흔들림 보호
		// 이탈 반경은 주행 평형(램프 0.05×이속 250 ≈ 12.5uu)보다 낮아야 함 — 높으면(25)
		// 매 틱 도달↔재견인이 교대돼 주입 속도가 540↔0으로 튀며 애니메이션이 발발거림.
		const float PairHoldExit   = 8.0f;
		// 직전 틱에 도달한 직후엔 재진입도 좁게 — 도달할 때마다 문턱이 70으로 돌아가면
		// 리더가 걷는 동안 25↔70 왕복(스톱-앤-고)이 남는다. 완전히 정착한 뒤에만 70 복귀.
		...
		// 1인 + 들것 피동: 들것에서 껐더니 상대 쪽으로 밀 때 가구가 몸을 관통해 겹쳤음
		// (운반 중 상호 충돌이 꺼져 있어 밀어냄 없이는 겹침을 못 막는다). 예전 '동료가 못
		// 따라오는 간섭'은 좁은 도달 반경(6uu) 히스테리시스가 원인이었고 한계 견인으로 해소.
		if (Players.Num() == 1 || bPairLineValid)
		{
			// 방향 기준은 피벗(ActualLoc)이 아니라 메시 바운즈 중심 — 피벗이 메시 밖에 있는
			// 가구는 피벗 기준이면 잡는 면에 따라 방향이 뒤집혀, 잡자마자 밀어내며 튕김.
		...
		// [입력 우선] 이동 입력 중인 운반자는 견인 대상에서 제외하고 능동으로 취급.
		// 견인 주입이 WASD를 매 틱 덮어쓰면 Δ가 닫히지도 벌어지지도 않는 줄다리기 교착
		// (Δ≈98에서 속도 0으로 정지)이 되어 '잡은 채 이동 불가'가 됨. 오토노머스 프록시의
		// Acceleration은 클라 ServerMove에 실려 오므로 서버에서도 유효하다.
		// (침범 밀어냄은 예외 — 입력 중이어도 가구 안으로 파고드는 것은 밀어내야 함)
		...
		if (IsCarryDebugEnabled() && GEngine)
		{
			// 선 = 플레이어→자기 대형 목표(Att). 초록=능동(간섭 없음) / 노랑=견인(속도 주입) /
			// 주황=도달 정지 틱(속도 0 주입) / 빨강=침범 밀어냄. 'WASD가 막힌다' = 능동이
			// 아닌 색이 자기 캐릭터에 계속 붙어 있다는 뜻이므로 그 색과 Δ값을 스크린샷.
		...
		// 회전 중(의도 >10°/s) 피동: 한계 안이면 완전 보류(축 유지), 밖이면 아래에서 초과분만
		// 소프트 견인. 이전의 '한계×2 절벽'은 보류↔전속 견인이 교대돼 Δ가 141↔75 널뛰는
		// 수평 텔레포트가 됐음(로그).
		...
		if (bAtTarget && bWasDragged)
		{
			...
			// [서버+클라 동시 주입] 원격 폰을 Multicast(소유 클라)만으로 움직이면: 클라가
			// 움직여도 서버는 ServerMove의 가속(0)을 자기 속도(0)에서 재적분해 제자리 →
			// 위치 불일치 → ClientAdjustPosition이 클라를 되돌려 견인이 무효가 된다
			// (Δ52→303 자동해제 로그). 서버와 소유 클라가 같은 값을 주입하면 move 재생
			// 결과가 일치해 보정이 발생하지 않는다.
			CMC->Velocity = FVector(0.0f, 0.0f, CMC->Velocity.Z);
			...
			// [들것 제외] 들것은 전원의 Yaw 제안이 선 목표로 일치해 재기록이 불필요하고,
			// 한계 견인의 도달 지점은 Slack(110uu) 벌어진 곳이라 재기록하면 늘어난 간격이
			// 새 대형으로 구워져 견인이 반복될수록 가구가 점점 멀어진다.
			if (!bPairLineValid)
			{
				FGrabAnchor& Anc        = Anchors[P];
				Anc.InitialOffset       = ActualLoc - P->GetActorLocation();
		...
		// ('끌려가는 양상'은 램프 지연이 아니라 위 회전 우선 견인이 담당 — 램프를 늘려
		//  트레일링을 만들면 회전 완료 후에도 늘어져서 오히려 둔하게 느껴짐)
		// 들것 램프 0.10: 0.05는 주입 속도가 수천 uu/s로 튀어 틱당 수십 uu 점프(텔레포트).
		// 평형 지연 25uu > 이탈 반경 8 → 주행 중 도달 채터링 없음(발발 방지 유지).
		...
		// 들것 견인 속도 상한 = 운반 이속 ×1.5 — 전속 주입이 텔레포트 체감의 주범
		...
		// [서버+클라 동시 주입] 위 도달 블록과 동일 — 서버가 안 쓰면 원격 폰 견인이 무효가 됨
	...
	for (ACharacter* P : ToRelease)
	{
		if (IsCarryDebugEnabled() && GEngine)
		{
			// '갑자기 놓쳤다' 스크린샷용: 해제 지점에 빨간 X(5초) + 사유 텍스트
	...
	ACharacter* Player, FVector CarryVelocity, float TargetYaw)
{
	...
	if (IsCarryDebugEnabled())
	{
		// 소유 클라 화면용: 지금 주입되는 견인 속도(파랑 화살표) / 정지 주입(빨강 점).
		// 'WASD가 안 먹는' 순간 자기 캐릭터에 이게 붙어 있으면 견인이 입력을 덮어쓰는 중.
